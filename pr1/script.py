...
def calculaPerplejidad(results):
    ...
    for submodelo in results.keys():
        for testSet in results[submodelo]:
            perplejidad = 2 ** -((1 / 1000) * sum(math.log2(prob)
                                 for prob in results[submodelo][testSet] if prob > 0))
...
if __name__ == '__main__':
    results = {}
    modelos = {"G1": ["G1-EQ", "G1-IS", "G1-SC"], "G2": ["G2-EQ",
                                                         "G2-IS", "G2-SC"], "G3": ["G3-EQ", "G3-IS", "G3-SC"]}
